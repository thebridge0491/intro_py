...
def build_sphinx(ctx, opts=''):
    ...
    #ctx.run("{0} -m sphinx.quickstart -p '{{cookiecutter.parentcap}}{{cookiecutter.joiner}}{{cookiecutter.projectcap}}' -a '{{cookiecutter.author}}' -v {{cookiecutter.version}} --no-batchfile --no-makefile --ext-autodoc --ext-viewcode -q docs".format(sys.executable), pty=True, shell='/bin/sh')
