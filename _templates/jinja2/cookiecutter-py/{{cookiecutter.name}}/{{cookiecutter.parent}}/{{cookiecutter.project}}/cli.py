# -*- coding: utf-8 -*-
'''Command line interface module for {{cookiecutter.parent}}{{cookiecutter.separator}}{{cookiecutter.project}}.
...
def run_{{cookiecutter.project}}(name):
    import re
...
def main(argv=None):
    ...
    logjson_str = {{cookiecutter.project}}.read_resource('logging.json', rsrc_path=rsrc_path)
    log_cfg = deserialize_str(logjson_str, fmt='json')
    {{cookiecutter.project}}.config_logging('info', 'cfg', log_cfg)
    opts_hash = parse_cmdopts(argv)
    {{cookiecutter.project}}.config_logging(opts_hash.log_lvl, opts_hash.log_opt, log_cfg)
    MODULE_LOGGER.info('main()')

    cfg_blank = {'hostname':'???', 'domain':'???', 'file1':{'path':'???', 
		'ext':'txt'}, 'user1':{'name':'???', 'age': -1}}
    cfg_ini = dict(cfg_blank.items())
    cfg_ini.update({{cookiecutter.project}}.ini_to_dict({{cookiecutter.project}}.read_resource('prac.conf',
		rsrc_path=rsrc_path)).items())
    cfg_json = dict(cfg_blank.items())
    cfg_json.update(deserialize_str({{cookiecutter.project}}.read_resource(
	'prac.json',
		rsrc_path=rsrc_path)).items())
    cfg_yaml = dict(cfg_blank.items())
    cfg_yaml.update(deserialize_str({{cookiecutter.project}}.read_resource(
	'prac.yaml',
		rsrc_path=rsrc_path), fmt='yaml').items())
    cfg_toml = dict(cfg_blank.items())
    cfg_toml.update(deserialize_str({{cookiecutter.project}}.read_resource(
	'prac.toml',
    ...
    print('')
    run_{{cookiecutter.project}}(opts_hash.user)
